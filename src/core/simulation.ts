...
const FALLOFF = Math.log(1 / TRANSMISSION_AT_TD);

/** How much light passes through `thickness` mm of a filament with this TD. */
export function transmission(thickness: number, td: number): number {
...
export function simulateLayers(
  ...
  if (lithophane) {
    ...
    for (let index = 0; index < layers; index++) {
      ...
      const t = transmission(layerThickness(index, layerHeight, firstLayerHeight), filament.td);
